...
__all__ = ['DyHeadBlock', 'FeaturePyramidSharedConv', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'C3_FocusedLinearAttention', 'C2f_FocusedLinearAttention', 'AKConv', 'C3_AKConv', 'C2f_AKConv']
...
class FeaturePyramidSharedConv(nn.Module):
    def __init__(self, c1, c2, dilations=[1, 3, 5]) -> None:
        ...
        self.share_conv = nn.Sequential(
            nn.Conv2d(in_channels=c_, out_channels=c_, kernel_size=3, stride=1, padding=1, groups=c_, bias=False),
            nn.Conv2d(c_, c_, 1, bias=False)  # Pointwise Conv
        )
        ...
        self.cv2 = Conv(c_ * (1 + len(dilations) + len(self.dilations1) + len(self.dilations2)), c2, 1, 1)
    ...
    def forward(self, x):
        ...
        for dilation in self.dilations:
            y.append(F.conv2d(y[-1], weight=self.share_conv[0].weight, bias=None, dilation=dilation,groups=y[-1].shape[1],
                              padding=(dilation * (3 - 1) + 1) // 2))


        for dilation1 in self.dilations1:
            y1.append(F.conv2d(y1[-1], weight=self.share_conv[0].weight, bias=None, dilation=dilation1,groups=y[-1].shape[1],
                               padding=(dilation1 * (3 - 1) + 1) // 2))


        for dilation2 in self.dilations2:
            y2.append(F.conv2d(y2[-1], weight=self.share_conv[0].weight, bias=None, dilation=dilation2,groups=y[-1].shape[1],
                               padding=(dilation2 * (3 - 1) + 1) // 2))


        del y1[0]
        del y2[0]
        fused_features = torch.cat(y + y1 + y2, dim=1)
...
class Bottleneck_DySnakeConv_custom(Bottleneck):
    """Standard bottleneck with DySnakeConv."""
...
class C2f_DySnakeConv_custom(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(Bottleneck_DySnakeConv_custom(self.c, self.c, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))
...
class Bottleneck_RFCAConv(Bottleneck):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):  # ch_in, ch_out, shortcut, groups, kernels, expand
        super().__init__(c1, c2, shortcut, g, k, e)
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, k[0], 1) # its was Conv GhostConv  RepLightConv RepConv EMSConv SCConv ScConv GSConv
