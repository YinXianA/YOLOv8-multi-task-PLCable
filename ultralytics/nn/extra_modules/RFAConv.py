...
class RFCAConv(nn.Module):
    def __init__(self, inp, oup, kernel_size, stride=1, reduction=32): # GSCONV c1, c2, k=1, s=1, p=None, g=1, d=1, act=True)
        ...
        self.generate = nn.Sequential(nn.Conv2d(inp,inp * (kernel_size**2),kernel_size,padding=kernel_size//2,
                                      ...
                                      nn.ReLU(inplace=True) #inplace=True added
    ...
    def forward(self, x):
        ...
        generate_feature = rearrange(generate_feature, 'b c (n1 n2) h w -> b c (h n1) (w n2)', n1=self.kernel_size,
                              n2=self.kernel_size)   #ADJUST SHAPE

        x_h = self.pool_h(generate_feature) #AVG POOL
        x_w = self.pool_w(generate_feature).permute(0, 1, 3, 2) #AVG POOL

        y = torch.cat([x_h, x_w], dim=2) #CONCAT
        y = self.conv1(y) #+CONV
        y = self.bn1(y)
        y = self.act(y)

        h,w = generate_feature.shape[2:]
        x_h, x_w = torch.split(y, [h, w], dim=2) #SPLIT

        x_w = x_w.permute(0, 1, 3, 2)

        a_h = self.conv_h(x_h).sigmoid() #CONV
        a_w = self.conv_w(x_w).sigmoid() #CONV
